...
class TwitchAccount(models.Model):
    ...
    def get_profile_url(self):
        return 'https://twitch.tv/' + self.extra_data.get('name')

    def get_avatar_url(self):
        return self.extra_data.get('logo')

    def get_name(self):
        return self.extra_data.get('display_name')


@python_2_unicode_compatible
class OAuth2AccessToken(models.Model):
    account = models.OneToOneField(TwitchAccount, on_delete=models.PROTECT)
